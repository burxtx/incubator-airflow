...
class CheckOperator(BaseOperator):
    ...
    * Empty dictionary or set (``{}``)
    ...
    def execute(self, context=None):
        ...
        source_records = self.get_source_db_hook().get_records(self.sql)
        logging.info('Executing HQL check: ' + self.hql)
        target_records = self.get_target_db_hook().get_records(self.hql)

        logging.info("Source record: " + str(source_records))
        logging.info("Target record: " + str(target_records))
        if not target_records:
            raise AirflowException("The query returned None")
        elif not all([bool(r) for r in target_records]):
            exceptstr = "Test failed.\nQuery:\n{q}\nResults:\n{r!s}"
            raise AirflowException(exceptstr.format(
                q=self.hql, r=target_records))
        if str(source_records) == str(target_records):
            logging.info("Success.")
        else:
            raise AirflowException("source and target row count unmatch")

    def get_source_db_hook(self):
        return BaseHook.get_hook(conn_id=self.source_conn_id)

    def get_target_db_hook(self):
        return BaseHook.get_hook(conn_id=self.target_conn_id)
